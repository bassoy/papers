...
\begin{itemize}
	...
	\item 
	Our algorithm with variable loop fusion and parallel slice-matrix multiplications is on average $17$\% faster than a single batched \tf{gemm} call when the contraction and leading dimensions are greater than $256$.
	%todo: quantify.
	\item 
	...
	%todo: slicing method with subtensors enables the use of a single gemm\_batch
	%The use of order-$2$ tensor slices helps to retain the performance at a peak level.
	\item
	Our \tf{LOG}-based tensor-times-matrix implementation are in general faster than \tf{TTGT}- and \tf{GETT}-based implementations that have been described in \cite{springer:2018:design, matthews:2018:high} including actively developed libraries such as Libtorch and Eigen.
