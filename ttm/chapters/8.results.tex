...
In (a) function version \tf{<gemm\_batch>} is executed,
in (b) and (e) \tf{<seq-loops,par-gemm>}, %
in (c) and (f) \tf{<par-loops,seq-gemm>}, %
in (d) and (g) \tf{<par-loops,par-gemm>}, %
Functions (b,c,d) and (e,f,g) are executed with tensor slices or subtensors, respectively.
...
We have first analyzed the two slicing methods by benchmarking function \tf{<par-loops,seq-gemm>} and \tf{<gemm-batch>} using asymmetrically (top) and symmetrically (bottom) shaped tensors.
%Note that this analysis is equal to minimizing or maximizing the input parameter $M_C$.
Fig. \ref{performance.tlib.contour} contains six filled contour plots (performance maps) in which \tf{<par-loops,seq-gemm>} either uses subtensors or tensor slices and \tf{<gemm-batch>} loops over subtensors only.
...
With symmetrically shaped tensors, the performance maps of \tf{<par-loops,seq-gemm>} with tensor slices and \tf{<gemm-batch>} are almost identical with a median performance of $221.52$ and $236.21$ Gflops/s.
Moreover, the slicing method does minimally affect the runtime behavior of \tf{<par-loops,seq-gemm>}.
...
This can by the fact that both dimensions are equal or larger than $4096$ which lets \tf{gemm} operate under optimal conditions.
