...
Our function \tf{<par-loops,seq-gemm>} with tensor slices is on average $11$\% faster than Intel's \tf{gemm\_batch} routine and reaches almost $1.1$ Tflops/s for non-edge cases with $q>2$ and $p > 6$.
This suggests that the Intel's implementation might not divide subtensors into smaller blocks.

With symmetrically shaped tensors, \tf{<par-loops,seq-gemm>} with tensor slices performs almost identical as \tf{<gemm-batch>} with $221.52$ Gflops/s and $236.21$ Gflops/s, respectively.
...
Fig. \ref{fig:performance.tlib.case8} contains cumulative performance distributions of all the proposed algorithms which are generated \tf{gemm} or \tf{gemm\_batch} calls of case 8.
...
For instance, function \tf{<seq-loops,par-gemm>} with subtensors computes the tensor-matrix product with equal to or less than 0.6 Tflops/s for $50$\% percent of the test cases using asymmetrically shaped tensor.
...
In case of asymmetrically shaped tensors, \tf{<par-loops,seq-gemm>} with tensor slices performs best and outperforms \tf{<gemm\_batch>} and \tf{<par-loops,seq-gemm>} with subtensors as previously stated.
One unexpected finding is that function \tf{<seq-loops,par-gemm>} with any slicing performs better than \tf{<gemm\_batch>} for certain tensor order and contraction mode combinations.
...
Functions \tf{<par-loops,seq-gemm>} and \tf{<gemm\_batch>} have almost the same performance distribution outperforming \tf{<seq-loops,par-gemm>} for almost every test case.
...
This observation might suggest that a multithreaded execution of \tf{gemm} 
