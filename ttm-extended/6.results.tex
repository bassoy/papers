...
If the leading and contraction dimensions are large, both versions of \ttt{<par-gemm>} outperform \ttt{<par-loop>} with any type of slicing. 
...
This subsection compares the runtime performance of the functions \ttt{<batched-gemm>} and \ttt{<combined>} against \ttt{<par-loop>} and \ttt{<par-gemm>} for the eighth TTM case.
...
In summary, \ttt{<combined>} performs as fast as, or faster than, \ttt{<par-gemm,subtensor>} and \ttt{<par-loop,slice>}, depending on the tensor shape. Conversely, the function \ttt{<batched-gemm>} underperforms for asymmetrically shaped tensors with large contraction modes and leading dimensions.
...
%TODO: improve on the percentage and tell how you computed the percentage: 100*(t(tlib)/t(other)). but now i would like to have ratio 100*(t(other)/t(tlib)-1) if t(other)>t(tlib) and 100*(t(tlib)/t(other)-1) otherwise.
...
The median library performances are up to $29.85$ GFLOPS/core and are thus at least $15.32$\% slower than TLIB.
...
Except for TBLIS, TLIB outperforms other libraries by a factor of at least $2$.
TBLIS computes the product with $9.84$ GFLOPS/core which is only $1.37$\% slower than TLIB.
...
On the AMD CPU, our implementation with AOCL computes the tensor-times-matrix product in median with $24.28$ GFLOPS/core ($1.55$ TFLOPS), reaching a maximum performance of $50.18$ GFLOPS/core ($3.21$ TFLOPS) with asymmetrically shaped tensors.
...
TLIB outperforms the remaining libraries which are at most reaching at most $62.97$\% percent of TLIB's throughputs.
...
It outperforms all other libraries and is at most $86.67$\% of TLIB's throughputs.
...
\begin{tabular}{lccc c}
	\toprule
	Library    & \multicolumn{3}{c}{Performance [GFLOPS/core]} & Ratio [\%] \\ 
	\cmidrule(lr){2-4}
	& Min   & Median  & Max   & \\ 
	\midrule
	TLIB       & $\mathbf{9.39}$  & $\mathbf{38.42}$   & $\mathbf{57.87}$ &           \\
	TCL        & $0.98$  & $24.16$   & $56.34$ & $84.69$     \\
	TBLIS      & $8.33$  & $29.85$   & $47.28$ & $80.62$     \\
	LibTorch   & $1.05$  & $28.68$   & $46.56$ & $78.00$     \\
	Eigen      & $5.85$  & $14.89$   & $15.67$ & $36.94$     \\
	\midrule
	TLIB       & $0.14$  & $8.99$    & $58.14$ &           \\
	TCL        & $0.04$  & $2.71$    & $56.63$ & $44.66$     \\
	TBLIS      & $\mathbf{1.11}$  & $\mathbf{9.84}$    & $45.03$ & $98.63$     \\
	LibTorch   & $0.07$  & $3.52$    & $\mathbf{62.20}$ & $53.32$     \\
	Eigen      & $0.21$  & $3.80$    & $16.06$ & $31.59$     \\
\bottomrule
\end{tabular}
\hspace{5em}
\begin{tabular}{lccc c}
	...
	TCL        & $0.61$  & $8.08$    & $41.82$ & $27.97$     \\
	TBLIS      & $\mathbf{9.06}$  & $\mathbf{26.81}$    & $43.83$ & $94.19$     \\
	LibTorch   & $0.63$  & $16.04$   & $50.84$ & $62.97$     \\
	Eigen      & $4.06$  & $11.49$   & $35.08$ & $54.65$     \\
	\midrule
	TLIB       & $0.02$  & $\mathbf{7.52}$    & $\mathbf{54.16}$ &           \\
	TCL        & $0.03$  & $2.03$    & $42.47$ & $44.94$     \\
	TBLIS      & $\mathbf{0.39}$  & $6.19$    & $41.11$ & $86.67$     \\
	LibTorch   & $0.05$  & $2.64$    & $46.65$ & $57.33$     \\
	Eigen      & $0.10$  & $5.58$    & $36.76$ & $69.72$     \\
	\bottomrule
\end{tabular}
\caption%
{%
\footnotesize
Performance comparison of TLIB with other tensor libraries.
The performance tests are run on the Intel Xeon Gold (left) and AMD EPYC CPU (right) where the upper/lower parts are tested with asymmetrically/symmetrically shaped tensors.
}
