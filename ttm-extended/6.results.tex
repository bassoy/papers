...
Note that we were able to link the current version of TuckerMPI only with Intel MKL, which is why TuckerMPI's TTM has only been executed on the Intel CPU.
\tbf{TLIB} denotes our library and the previously discussed \tf{<combined>} algorithm.
%All of the following provided performance and comparison values are median values.
...
Using MKL on the Intel CPU, TLIB achieves a median performance of $38.21$ GFLOPS/core ($1.83$ TFLOPS) and reaches a maximum performance of $57.68$ GFLOPS/core ($2.76$ TFLOPS) with asymmetrically shaped tensors from the shape set $N$.
TLIB is many tensor instances faster than other libraries and therefore achieves a speedup of at least $6.36$\%.
LibTorch and TuckerMPI have almost the same median performance of $38.17$ and $35.98$ GFLOPS/core, yet only reach a peak performance $50.48$ and $53.21$ GFLOPS/core.
Both are in median $17.47$\% and $6.97$\% slower than TLIB.
In case of symmetrically shaped tensors, TLIB's median performance is $8.99$ GFLOPS/core ($431.52$ GFLOPS).
%TODO: hier weiter.
...
Note that the multiplication over the first and fourth mode corresponds to the sixth and seventh TTM case in Table \ref{tab:mapping_rm_cm}.
TLIB will call a single \ttt{gemm} for both cases.
The multiplication over the second and third mode corresponds to the eighth TTM case.
Fig. \ref{fig:performance-sdr.comparison} contains bar plots for all tensor shapes of set $Q$.
The size of each bar is the total running time of the respective TTM implementation that is executed on an Intel Xeon Gold 5318Y CPU and an AMD EPYC 9354 CPU.
Note that TCL was not able to compute the TTM for the EXAFEL data set which is why the runtime is set to zero.

On the Intel Xeon Gold 5318Y CPU, TLIB is in most cases faster than other libraries and outperforms them by at most $137.32$\% (TCL), $100.80$\% (TBLIS), $210.71$\% (LibTorch), $798.91$\% (Eigen), $581.73$\% (TuckerMPI).
TCL is on par with TLIB for the CESM-ATM data set.
TuckerMPI is $46.8$\% (CESM-ATM) and $13.7$\% (Miranda) faster than TLIB.
More precisely, the TTMs of TuckerMPI and LibTorch over the fourth mode is faster than TLIB's TTM for almost all tensor instances.

On the AMD EPYC 9354 CPU, TLIB performs better than most other libraries except for TCL and LibTorch in some instances.
TLIB is at most $33.36$\% (TCL), $117.22$\% (TBLIS), $221.25$\% (LibTorch), $205.80$\% (Eigen) faster.
TCL outerperforms TLIB by $16.22$\% (NYX) and $71.65$\% (Miranda).
In this case, TCL's TTM over the fourth mode outperforms TLIB's TTM for almost all tensor instances and is in case of the SCALE-LETKF data set $3.4$x faster.
...
The runtime tests with tensors from the SDRBench illustrates that TLIB is able to compute the TTM faster than other libraries for most tensor shapes.
