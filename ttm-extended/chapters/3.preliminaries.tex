...
CBLAS denotes the C interface to the BLAS that can be used by C or C++ programs where each C function is prefixed with \ttt{cblas}.
For the sake of simplicity, we omit the \ttt{cblas} prefix of CBLAS functions.
The following algorithms implementing the tensor-matrix multiplication uses the general matrix-matrix multiplication \ttt{gemm} and only for one special case the matrix-vector multiplications \ttt{gemv}.
Function \ttt{gemm} is defined as \ttt{C:=a*op(A)*op(B)+b*C} where 
...
The BLAS interfaces also allow the user to specify the leading dimension of a matrix by providing the \ttt{lda}, \ttt{ldb} and \ttt{ldc} parameters.
...
The CBLAS interface also provides an additional layout parameter \ttt{CBLAS\_LAYOUT} with which one can specify the column-major or row-major ordering.
